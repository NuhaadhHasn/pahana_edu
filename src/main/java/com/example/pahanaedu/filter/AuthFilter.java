...
public class AuthFilter implements Filter {
    ...
    private static final List<String> PUBLIC_PATHS = Arrays.asList("/login", "/logout", "/help.jsp");
    private static final List<String> ADMIN_PATHS = Arrays.asList("/users", "/promotions", "/login-history", "/reports", "/view-bill-details");
    private static final List<String> STAFF_PATHS = Arrays.asList("/customers", "/items", "/billing", "/bill-history", "/record-payment");
    private static final List<String> CUSTOMER_PATHS = Arrays.asList("/customer-dashboard.jsp", "/my-bills", "/view-items", "/make-payment", "/view-bill-details", "/customer-make-payment", "/items");
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        if (path.startsWith("/login") || path.endsWith(".css") || path.endsWith(".js") || path.startsWith("/images/")) {
        ...
        if ("ADMIN".equals(userRole)) {
            ...
            if (path.startsWith("/dashboard.jsp") || isPathAllowed(path, ADMIN_PATHS) || isPathAllowed(path, STAFF_PATHS) || isPathAllowed(path, SHARED_API_PATHS) || isPathAllowed(path, PUBLIC_PATHS)) {
                isAuthorized = true;
            }
        } else if ("STAFF".equals(userRole)) {
            // Staff can access STAFF, SHARED_API paths, and their dashboard.
            if (path.startsWith("/dashboard.jsp") || isPathAllowed(path, STAFF_PATHS) || isPathAllowed(path, SHARED_API_PATHS) || isPathAllowed(path, PUBLIC_PATHS)) {
                isAuthorized = true;
            }
        } else if ("CUSTOMER".equals(userRole)) {
            // Customer can only access CUSTOMER paths.
            if (isPathAllowed(path, CUSTOMER_PATHS) || isPathAllowed(path, PUBLIC_PATHS)) {
                isAuthorized = true;
            }
        }
